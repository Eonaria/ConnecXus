...
export default defineEventHandler(async (event) => {
  ...
  if (!isAllowed) {
    throw createError({ statusCode: 403, message: 'Forbidden: เฉพาะผู้สร้างกลุ่มและผู้ดูแลเท่านั้นที่สามารถส่งต่อรายงานได้' })
  }

  // Escalate the report
  const [result] = await db.query<ResultSetHeader>(
    "UPDATE reports SET is_escalated = 1, status = 'pending' WHERE id = ? AND community_id = ?",
